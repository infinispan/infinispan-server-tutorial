...
To create and run query on the `weather` cache:

. Get the `QueryFactory` from the cache
. Create a query using 'Ickle'. Find every `LocationWeather` in a country.
...
. Get the `QueryFactory` from the cache
. Create a query using 'Ickle'. Find every `LocationWeather` with a weather condition and return only the city.
. Set the `condition` parameter
. Execute the query, return the list and filter the `Object[]` to get those `String`
