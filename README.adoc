...
[IMPORTANT]
By default, Infinispan Server *requires user authentication* and *authorization* is implicit since
For the purposes of this tutorial, create a user named `admin` so you get administrative access to Infinispan Server.
You can find more details about users and permissions in https://infinispan.org/docs/stable/titles/server/server.html#creating-users_quickstart[Creating and Modifying Users]
...
* Podman
+
`podman run --net=host -p 11222:11222 -e USER="admin" -e PASS="password" quay.io/infinispan/server:12.1`

* Docker
+
`docker run -it -p 11222:11222 -e USER="admin" -e PASS="password" infinispan/server:12.1`
...
. Enter your credentials (`admin`/`password`).
